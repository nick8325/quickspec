...
-- XXX introduce typed term type with context+maybe result type
-- means that we can't have two variables with same index but different type.
-- but this is pretty dodgy with polymorphism anyway.
-- maybe explicitly compute mapping from vars -> var numbers when testing?
