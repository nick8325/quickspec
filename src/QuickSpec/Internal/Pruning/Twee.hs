...
data Extended fun = Minimal | Skolem Twee.Var | Function fun | Tag Type | Apply
...
instance (Ord fun, Typeable fun, Pretty fun) => Pretty (Extended fun) where
  ...
  pPrintPrec _ _ Apply = text "$"
  pPrintPrec _ _ (Tag ty) = text "@" <#> brackets (pPrint ty)
...
toTwee = toTweeWith Twee.var

skolemise :: (Typed f, Ord f, Typeable f) =>
  Term f -> Twee.Term (Extended f)
skolemise = toTweeWith (Twee.con . Twee.fun . Skolem)

{-# INLINE toTweeWith #-}
toTweeWith :: (Typed f, Ord f, Typeable f) =>
  (Twee.Var -> Twee.Builder (Extended f)) ->
  Term f -> Twee.Term (Extended f)
toTweeWith var = Twee.build . tt
  where
    tt t =
      Twee.app (Twee.fun (Tag ty)) (tt1 t)
      where
        ty = typ t
    tt1 (Var (V _ x)) =
      var (Twee.V x)
    tt1 (Fun f) =
      Twee.con (Twee.fun (Function f))
    tt1 (t :$: u) =
      Twee.app (Twee.fun Apply) [tt t, tt u]
